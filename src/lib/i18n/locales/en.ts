export const en = {
  appName: 'Vistoria',
  tabs: {
    home: 'Home',
    library: 'Library',
    search: 'Search',
    create: 'Create',
    profile: 'Profile',
  },
  home: {
    title: 'Home',
    recentStories: 'Recent stories',
    collections: 'Collections',
    emptyTitle: 'Your archive is empty',
    emptySubtitle: 'Capture a memory or document a process to get started.',
    signInTitle: 'Sign in to see your archive',
    signInSubtitle: 'Create an account or sign in to start capturing memories.',
  },
  library: {
    title: 'Library',
    grid: 'Grid',
    list: 'List',
    storiesCount: 'stories',
    emptyTitle: 'Nothing here yet',
    emptySubtitle: 'Folders and stories you create will show up here.',
    signInTitle: 'Sign in to see your library',
    signInSubtitle: 'Create an account or sign in to organize your stories.',
  },
  search: {
    title: 'Search',
    placeholder: 'Search stories, tags, notes…',
    emptyTitle: 'Search your archive',
    emptySubtitle: 'Find stories by title, tag, folder, or slide text.',
    noResultsTitle: 'No matches',
    noResultsSubtitle: 'Try a different word or tag.',
  },
  create: {
    title: 'Create',
    newStory: 'New Story',
    newStorySubtitle: 'Capture photos, videos, and notes as slides',
    newFolder: 'New Folder',
    newFolderSubtitle: 'Organize stories into a collection',
    signInTitle: 'Sign in to create',
    signInRequired: 'Sign in to create folders and stories.',
  },
  newFolder: {
    title: 'New Folder',
    namePlaceholder: 'Folder name',
    create: 'Create folder',
  },
  newStory: {
    title: 'New Story',
    titlePlaceholder: 'Story title',
    create: 'Create story',
  },
  storyDetail: {
    descriptionPlaceholder: 'No description yet.',
    slidesTitle: 'Slides',
    noSlidesTitle: 'No slides yet',
    noSlidesSubtitle: 'Add a photo, video, or text slide below.',
    deleteStory: 'Delete story',
    deleteConfirmTitle: 'Delete this story?',
    deleteConfirmBody: 'This cannot be undone.',
    notFound: 'Story not found',
    addCamera: 'Camera',
    addVideo: 'Video',
    addGallery: 'Gallery',
    addText: 'Text',
    uploading: 'Uploading…',
    slideTextPlaceholder: 'Slide text',
    slideCaptionPlaceholder: 'Add a caption…',
    deleteSlideConfirmTitle: 'Delete this slide?',
    tagsTitle: 'Tags',
    tagPlaceholder: 'Add a tag…',
    commentsTitle: 'Comments',
    commentPlaceholder: 'Add a comment…',
    noCommentsTitle: 'No comments yet',
    noCommentsSubtitle: 'Be the first to say something.',
  },
  viewer: {
    edit: 'Edit',
    comment: 'Comment',
    share: 'Share',
    info: 'Info',
    comingSoon: 'Coming soon',
    noSlidesTitle: 'This story has no slides yet',
    noSlidesEdit: 'Add slides',
  },
  share: {
    title: 'Share this story',
    createLink: 'Create share link',
    copyLink: 'Copy link',
    copied: 'Link copied',
    deactivate: 'Turn off link',
    reactivate: 'Turn link back on',
    inactiveNote: 'This link is off — no one can open it right now.',
    linkAccessNote: 'Anyone with this link can view the story.',
  },
  team: {
    title: 'Team',
    inviteEmailPlaceholder: "Invite by email",
    roleEditor: 'Editor',
    roleViewer: 'Viewer',
    invite: 'Invite',
    owner: 'Owner',
    removeConfirmTitle: 'Remove this person from the story?',
    userNotFound: 'No account found with that email.',
  },
  profile: {
    title: 'Profile',
    account: 'Account',
    language: 'Language',
    appearance: 'Appearance',
    signOut: 'Sign out',
    signIn: 'Sign in',
    notSignedIn: 'Not signed in',
  },
  auth: {
    signInTitle: 'Sign in',
    signUpTitle: 'Create account',
    email: 'Email',
    password: 'Password',
    signIn: 'Sign in',
    signUp: 'Create account',
    noAccount: "Don't have an account? Create one",
    haveAccount: 'Already have an account? Sign in',
    notConfiguredTitle: 'Supabase not configured',
    notConfiguredBody: 'Add EXPO_PUBLIC_SUPABASE_URL and EXPO_PUBLIC_SUPABASE_ANON_KEY to .env, then restart the app.',
    checkEmailToConfirm: 'Account created. Check your email to confirm it, then sign in.',
  },
  common: {
    cancel: 'Cancel',
    save: 'Save',
    delete: 'Delete',
    retry: 'Retry',
    comingSoon: 'Coming soon',
    error: 'Something went wrong.',
  },
};

export type TranslationSchema = typeof en;
